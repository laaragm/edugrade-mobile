...
export async function GetTeacherById(id: number): Promise<IServiceResponse<ITeacher>> {
    // const route = `/teachers/${id}`;
    ...
    const teacher = mockedTeachers.find((x) => x.id === id);
    return {
        result: !!teacher ? teacher : mockedTeachers[0],
        error: false,
        errorMessage: "",
    };
}
