import { IServiceResponse, ISubject, ISubjects } from "@/models";
...
export async function GetAllSubjects(): Promise<IServiceResponse<ISubjects>> {
    const route = "/subjects";
    const defaultErrorMessage = "Não foi possível recuperar os dados. Tente novamente mais tarde.";
    const result = await Get<ISubjects>({ route, defaultErrorMessage });
